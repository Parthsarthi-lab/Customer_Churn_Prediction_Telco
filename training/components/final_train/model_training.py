...
class ModelTrainer:
    ...
    def filter_hyperparams(params):
        ...
        hyperparams = {key.replace('classifier__', ''): value for key, value in params.items() if key.startswith('classifier__')}
        return hyperparams
    ...
    def train_model(self, xtrain, xtest, ytrain, ytest):
        try:
            ...
            # Filter the hyperparameters of the RandomForestClassifier model
            hyperparams = self.filter_hyperparams(hyperparams)

            # Initialize and train the RandomForestClassifier
            final_model = RandomForestClassifier(random_state=42, **hyperparams)
            ...
            info_logger.info(f"Final model trained with test accuracy: {test_accuracy}")
